...
,@i__itemBulk varchar(MAX) = '07119905032'
...
if object_id('tempdb..#tbl_itemH') is not null
...
if object_id('tempdb..#stockItem3') is not null
...
(
		...
		and _sr.stockQty > 0
		AND isnull(_s.consignCustCode,'') not in ('ㅇ499', 'ㅂ022', 'ㅇ479', 'ㅇ002', 'ㅇ496', 'ㅂ184')
) locaMemo ,
st.comCode,
st.itemId,
i.itemNo,
...
CROSS APPLY (
	...
	AND (@i__consignCustCode = '' 
	  ...
	  or (_s.consignCustCode is null AND _s.comCode = @i__consignCustCode))
	AND isnull(_s.consignCustCode,'') not in ('ㅇ499', 'ㅂ022', 'ㅇ496', 'ㅇ479', 'ㅇ002', 'ㅂ184', 'ㅈ011')	
...
CROSS APPLY (
	...
	and _r.validYN = 'Y' and ISNULL(_s.rlStandByYN,'N') <> 'Y' 
	and _s.validYN = 'Y' AND _s.storType in ('신품','중고','리퍼') AND _s.workableYN = 'Y'
	AND _s.consignCustCode in ('ㅇ499', 'ㅂ022', 'ㅇ496', 'ㅇ479', 'ㅇ002', 'ㅂ184', 'ㅈ011')
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty1 --보스카통상 재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode 
	  AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode 
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty2 --보스카통상 가용재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode 
	  AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode 
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty3 --보스카통상,이지통상 제외 가용재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode 
	  AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode 
