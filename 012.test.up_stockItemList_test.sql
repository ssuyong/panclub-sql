...
DECLARE @sql nvarchar(max), @sqlS nvarchar(max) = N'' , @sqlF nvarchar(max) = N'' 
	  ...
	  , @sql6 nvarchar(max) = N'' , @sql7 nvarchar(max) = N''
SET @sql = N''
...
SET @sql1 = @sql1 + N'
	(
		...
		where _sr.itemid = st.itemId 
		  ...
		  and _sr.stockQty > 0
		  AND ISNULL(_s.consignCustCode,'''') NOT IN (''ㅇ499'', ''ㅂ022'', ''ㅇ479'', ''ㅇ002'', ''ㅂ184'', ''ㅈ011'', ''ㅇ496'')
	'
	--if @i__logUserId = 'zzz'
	--	SET @sql = @sql + N'	and _s.storageCode <> ''zzz'' '
...
CROSS APPLY (
	...
	AND _s.storType in (''신품'',''중고'',''리퍼'') 
	AND _s.workableYN = ''Y''
	AND _s.consignCustCode in (''ㅇ499'', ''ㅂ022'', ''ㅇ479'', ''ㅇ002'', ''ㅂ184'', ''ㅈ011'', ''ㅇ496'')
