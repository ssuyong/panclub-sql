...
@i__orderBy='',    @i__sYmd1='',      @i__eYmd1='',      @i__sYmd2='', 
@i__eYmd2='',        @i__storCode='',    @i__itemId=0,    @i__itemNo='',    @i__itemName='',    @i__makerCode='',    @i__classCode='',    @i__storName='',    
@i__bulkSrchType='itemNo',    @i__itemBulk='2054600696',    @i__checkType='ALL',    @i__outStorCode='',    @i__storageCode='',    @i__noRealYN='N', 
@i__qtyZeroYN='N',    @i__consignCustCode='',      @i__logComCode='ㅇ499',    @i__logUserId='ssuyong'

--테스트:제파재고 07119905032
panErp.dbo.up_stockItemList_test	@i__workingType='SALE_LIST',    @i__page=0,    @i__qty=0,      
@i__orderBy='',    @i__sYmd1='',      @i__eYmd1='',      @i__sYmd2='', 
@i__eYmd2='',        @i__storCode='',    @i__itemId=0,    @i__itemNo='',    @i__itemName='',   
@i__makerCode='',    @i__classCode='',    @i__storName='',    
@i__bulkSrchType='itemNo',    @i__itemBulk='07119905032',    @i__checkType='ALL',    @i__outStorCode='',    
@i__storageCode='',    @i__noRealYN='N', 
@i__qtyZeroYN='N',    @i__consignCustCode='',      @i__logComCode='ㄱ121',    @i__logUserId='ssuyong'

--개발:부품인재고 07119905032
panErp.dbo.up_stockItemList_test	@i__workingType='SALE_LIST',    @i__page=0,    @i__qty=0,      
@i__orderBy='',    @i__sYmd1='',      @i__eYmd1='',      @i__sYmd2='', 
@i__eYmd2='',        @i__storCode='',    @i__itemId=0,    @i__itemNo='',    @i__itemName='',   
@i__makerCode='',    @i__classCode='',    @i__storName='',    
...
DECLARE @sql nvarchar(max), @sqlS nvarchar(max) = N'' , @sqlF nvarchar(max) = N'' 
	  ...
	  , @sql6 nvarchar(max) = N'' , @sql7 nvarchar(max) = N''   , @sql8 nvarchar(max) = N''
	  , @sql9 nvarchar(max) = N''
SET @sql = N''
...
SET @sql1 = @sql1 + N'
	(
		...
		where _sr.itemid = st.itemId 
		  ...
		  and _sr.stockQty > 0
		  AND ISNULL(_s.consignCustCode,'''') NOT IN (''ㅇ499'', ''ㅂ022'', ''ㅇ479'', ''ㅇ002'', ''ㅂ184'', ''ㅈ011'', ''ㅂ186'', ''ㄱ008'', ''ㅇ496'')
	'
	--if @i__logUserId = 'zzz'
	--	SET @sql = @sql + N'	and _s.storageCode <> ''zzz'' '
...
CROSS APPLY (
	...
	AND _s.storType in (''신품'',''중고'',''리퍼'') 
	AND _s.workableYN = ''Y''
	AND _s.consignCustCode in (''ㅇ499'', ''ㅂ022'', ''ㅇ479'', ''ㅇ002'', ''ㅂ184'', ''ㅈ011'', ''ㅂ186'', ''ㄱ008'', ''ㅇ496'')
...
,''ㅂ184'' AS otherSaleType

,ISNULL(osr.purRate , 0) AS saleRate
'
IF @i__itemBulk <> ''
BEGIN
    SET @sql6 = @sql6 + N' , ISNULL(bk.idx, NULL) AS bk_idx '
END
SET @sql6 = @sql6 + N' 

,ca3.qty3 AS qty3

,''ㅂ184'' AS stockRackCode
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty1 --보스카 재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode AND _s.storageCode = _r.storageCode 
	where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty2 --보스카 가용재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode AND _s.storageCode = _r.storageCode 
	where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty3 --보스카,이지통상 제외 가용재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode AND _s.storageCode = _r.storageCode 
	where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
	...
	and _s.validYN = ''Y'' AND _s.storType in (''신품'',''중고'',''리퍼'') 
	AND _s.workableYN = ''Y''
	AND ISNULL(_s.consignCustCode,'''') <> ''ㅂ184''
	AND ISNULL(_s.consignCustCode,'''') <> ''ㅇ496''
...
BEGIN
	IF @i__bulkSrchType = 'itemId' 
		SET @sql6 = @sql6 + '	INNER JOIN (SELECT srchKeyWord, MIN(idx) idx 
		                                    FROM #tbl_itemH 
											GROUP BY srchKeyWord) bk ON i.ItemId = bk.srchKeyword '
...
IF @i__storageCode <> '' 
BEGIN
	SET @sql6 = @sql6 + '	JOIN #stockItem3 st2 ON st2.comCode = st.comCode AND st2.itemId = st.itemId '
END
...
IF @i__logComCode NOT IN (SELECT comCode FROM dbo.UF_GetChildComcode(@ErpOperateComCode))  
BEGIN
	SET @sql6 = @sql6 + '	LEFT OUTER JOIN dbo.e_stockItemOuterNonDsp nd ON st.comCode = nd.comCode 
	                          AND st.itemId = nd.itemId '
END
...
IF @i__checkType = 'OUT'
BEGIN
	SET @sql6 = @sql6 + N' AND 1=1 '
END
ELSE IF @i__checkType = 'ALL' --외부재고다중조회
BEGIN
	SET @sql6 = @sql6 + N' AND @n__4carComCode = st.comCode  '
END
ELSE
BEGIN
	SET @sql6 = @sql6 + N' AND st.comCode = @i__logComCode '
END
...
IF @i__itemId <> ''
SET @sql6 = @sql6 + N' AND st.itemId= @i__itemId '

IF @i__itemNo <> ''
SET @sql6 = @sql6 + N' AND i.itemNo LIKE ''%'+@i__itemNo+'%'' '	

--IF @i__storName <> ''
--SET @sqlW = @sqlW + N'  AND sg.storageName LIKE '''+@i__storName+'%'' '	

IF @i__makerCode <> ''
SET @sql6 = @sql6 + N' AND i.makerCode= @i__makerCode '

IF @i__itemName <> ''
SET @sql6 = @sql6 + N' AND (i.itemName LIKE ''%'+@i__itemName+'%'' OR i.itemNameEn LIKE ''%'+@i__itemName+'%'' ) '

IF @i__classCode <> ''
SET @sql6 = @sql6 + N' AND i.classCode= @i__classCode '

IF @i__ymdIgnoreYN <> 'Y' AND @i__sYmd1 <> ''
	SET @sql6 = @sql6 + N' AND st.uptYmd BETWEEN @i__sYmd1 AND @i__eYmd1 '

IF @i__noRealYN = 'Y'
	SET @sql6 = @sql6 + N' AND  ISNULL(noRealYN, ''N'') <> ''Y'''
IF @i__qtyZeroYN = 'Y'
	SET @sql6 = @sql6 + N' AND ISNULL(str.qtyWorkable, 0) <> 0 AND ISNULL(st.stockQty, 0) <> 0 '

--위에서 이걸로 변경. 2024.10.16 hsg
IF @i__logComCode NOT IN (SELECT comCode FROM dbo.UF_GetChildComcode(@ErpOperateComCode))   
BEGIN
	SET @sql6 = @sql6 + N' AND	nd.itemId IS NULL'
END
-------------------------------------------------------------------------------------------------------------------------------------------------------------ssy
-------------------------------------------------------------------------------------------------------------------------------------------------------------ssy
--sql7: 제파 재고
	...
	(
		...
		where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
		  AND (@i__consignCustCode = '''' or _s.consignCustCode = @i__consignCustCode 
		    or (_s.consignCustCode is null AND _s.comCode = @i__consignCustCode))
		  and _sr.stockQty > 0
		  AND _s.consignCustCode = ''ㅈ011''
	'
	--if @i__logUserId = 'zzz'
	--	SET @sql = @sql + N'	and _s.storageCode <> ''zzz'' '
	SET @sql7 = @sql7 + N'	) locaMemo ,  '
END
ELSE
BEGIN
SET @sql7 = @sql7 + N''''' locaMemo ,'
...
,''ㅈ011'' AS otherSaleType

,ISNULL(osr.purRate , 0) AS saleRate
'
IF @i__itemBulk <> ''
BEGIN
    SET @sql7 = @sql7 + N' , ISNULL(bk.idx, NULL) AS bk_idx '
END
SET @sql7 = @sql7 + N' 

,ca3.qty3 AS qty3

,''ㅈ011'' AS stockRackCode
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty1 --제파 재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode AND _s.storageCode = _r.storageCode 
	where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty2 --제파 가용재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode AND _s.storageCode = _r.storageCode 
	where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty3 --제파,이지통상 제외 가용재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode AND _s.storageCode = _r.storageCode 
	where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
	...
	and _s.validYN = ''Y'' AND _s.storType in (''신품'',''중고'',''리퍼'') 
	AND _s.workableYN = ''Y''
	AND ISNULL(_s.consignCustCode,'''') <> ''ㅈ011''
	AND ISNULL(_s.consignCustCode,'''') <> ''ㅇ496''
...
BEGIN
	IF @i__bulkSrchType = 'itemId' 
		SET @sql7 = @sql7 + '	INNER JOIN (SELECT srchKeyWord, MIN(idx) idx 
		                                    FROM #tbl_itemH 
											GROUP BY srchKeyWord) bk ON i.ItemId = bk.srchKeyword '
...
IF @i__storageCode <> '' 
BEGIN
	SET @sql7 = @sql7 + '	JOIN #stockItem3 st2 ON st2.comCode = st.comCode AND st2.itemId = st.itemId '
END
...
IF @i__logComCode NOT IN (SELECT comCode FROM dbo.UF_GetChildComcode(@ErpOperateComCode))  
BEGIN
	SET @sql7 = @sql7 + '	LEFT OUTER JOIN dbo.e_stockItemOuterNonDsp nd ON st.comCode = nd.comCode 
	                          AND st.itemId = nd.itemId '
END
...
IF @i__checkType = 'OUT'
BEGIN
	SET @sql7 = @sql7 + N' AND 1=1 '
END
ELSE IF @i__checkType = 'ALL' --외부재고다중조회
BEGIN
	SET @sql7 = @sql7 + N' AND @n__4carComCode = st.comCode  '
END
ELSE
BEGIN
	SET @sql7 = @sql7 + N' AND st.comCode = @i__logComCode '
END
...
IF @i__itemId <> ''
SET @sql7 = @sql7 + N' AND st.itemId= @i__itemId '

IF @i__itemNo <> ''
SET @sql7 = @sql7 + N' AND i.itemNo LIKE ''%'+@i__itemNo+'%'' '	

--IF @i__storName <> ''
--SET @sqlW = @sqlW + N'  AND sg.storageName LIKE '''+@i__storName+'%'' '	

IF @i__makerCode <> ''
SET @sql7 = @sql7 + N' AND i.makerCode= @i__makerCode '

IF @i__itemName <> ''
SET @sql7 = @sql7 + N' AND (i.itemName LIKE ''%'+@i__itemName+'%'' OR i.itemNameEn LIKE ''%'+@i__itemName+'%'' ) '

IF @i__classCode <> ''
SET @sql7 = @sql7 + N' AND i.classCode= @i__classCode '

IF @i__ymdIgnoreYN <> 'Y' AND @i__sYmd1 <> ''
	SET @sql7 = @sql7 + N' AND st.uptYmd BETWEEN @i__sYmd1 AND @i__eYmd1 '

IF @i__noRealYN = 'Y'
	SET @sql7 = @sql7 + N' AND  ISNULL(noRealYN, ''N'') <> ''Y'''
IF @i__qtyZeroYN = 'Y'
	SET @sql7 = @sql7 + N' AND ISNULL(str.qtyWorkable, 0) <> 0 AND ISNULL(st.stockQty, 0) <> 0 '

--위에서 이걸로 변경. 2024.10.16 hsg
IF @i__logComCode NOT IN (SELECT comCode FROM dbo.UF_GetChildComcode(@ErpOperateComCode))   
BEGIN
	SET @sql7 = @sql7 + N' AND	nd.itemId IS NULL'
END
-------------------------------------------------------------------------------------------------------------------------------------------------------------ssy
-------------------------------------------------------------------------------------------------------------------------------------------------------------ssy
--sql8: 부품인 재고
	...
	(
		...
		where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
		  AND (@i__consignCustCode = '''' or _s.consignCustCode = @i__consignCustCode 
		    or (_s.consignCustCode is null AND _s.comCode = @i__consignCustCode))
		  and _sr.stockQty > 0
		  AND _s.consignCustCode = ''ㅂ186''--부품인
	'
	--if @i__logUserId = 'zzz'
	--	SET @sql = @sql + N'	and _s.storageCode <> ''zzz'' '
	SET @sql8 = @sql8 + N'	) locaMemo ,  '
END
ELSE
BEGIN
SET @sql8 = @sql8 + N''''' locaMemo ,'
...
,''ㅂ186'' AS otherSaleType

,ISNULL(osr.purRate , 0) AS saleRate
'
IF @i__itemBulk <> ''
BEGIN
    SET @sql8 = @sql8 + N' , ISNULL(bk.idx, NULL) AS bk_idx '
END
SET @sql8 = @sql8 + N' 

,ca3.qty3 AS qty3

,''ㅂ186'' AS stockRackCode
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty1 --부품인 재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode AND _s.storageCode = _r.storageCode 
	where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
	AND (@i__consignCustCode = '''' or _s.consignCustCode = @i__consignCustCode 
	  or (_s.consignCustCode is null AND _s.comCode = @i__consignCustCode))
	AND _s.consignCustCode = ''ㅂ186''
    
) ca1

CROSS APPLY (
	select sum(_sr.stockQty) AS qty2 --부품인 가용재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode AND _s.storageCode = _r.storageCode 
	where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty3 --부품인,이지통상 제외 가용재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode AND _s.storageCode = _r.storageCode 
	where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
	...
	and _s.validYN = ''Y'' AND _s.storType in (''신품'',''중고'',''리퍼'') 
	AND _s.workableYN = ''Y''
	AND ISNULL(_s.consignCustCode,'''') <> ''ㅂ186''
	AND ISNULL(_s.consignCustCode,'''') <> ''ㅇ496''
...
BEGIN
	IF @i__bulkSrchType = 'itemId' 
		SET @sql8 = @sql8 + '	INNER JOIN (SELECT srchKeyWord, MIN(idx) idx 
		                                    FROM #tbl_itemH 
											GROUP BY srchKeyWord) bk ON i.ItemId = bk.srchKeyword '
...
IF @i__storageCode <> '' 
BEGIN
	SET @sql8 = @sql8 + '	JOIN #stockItem3 st2 ON st2.comCode = st.comCode AND st2.itemId = st.itemId '
END
...
IF @i__logComCode NOT IN (SELECT comCode FROM dbo.UF_GetChildComcode(@ErpOperateComCode))  
BEGIN
	SET @sql8 = @sql8 + '	LEFT OUTER JOIN dbo.e_stockItemOuterNonDsp nd ON st.comCode = nd.comCode 
	                          AND st.itemId = nd.itemId '
END
...
IF @i__checkType = 'OUT'
BEGIN
	SET @sql8 = @sql8 + N' AND 1=1 '
END
ELSE IF @i__checkType = 'ALL' --외부재고다중조회
BEGIN
	SET @sql8 = @sql8 + N' AND @n__4carComCode = st.comCode  '
END
ELSE
BEGIN
	SET @sql8 = @sql8 + N' AND st.comCode = @i__logComCode '
END
...
IF @i__itemId <> ''
SET @sql8 = @sql8 + N' AND st.itemId= @i__itemId '

IF @i__itemNo <> ''
SET @sql8 = @sql8 + N' AND i.itemNo LIKE ''%'+@i__itemNo+'%'' '	

--IF @i__storName <> ''
--SET @sqlW = @sqlW + N'  AND sg.storageName LIKE '''+@i__storName+'%'' '	

IF @i__makerCode <> ''
SET @sql8 = @sql8 + N' AND i.makerCode= @i__makerCode '

IF @i__itemName <> ''
SET @sql8 = @sql8 + N' AND (i.itemName LIKE ''%'+@i__itemName+'%'' OR i.itemNameEn LIKE ''%'+@i__itemName+'%'' ) '

IF @i__classCode <> ''
SET @sql8 = @sql8 + N' AND i.classCode= @i__classCode '

IF @i__ymdIgnoreYN <> 'Y' AND @i__sYmd1 <> ''
	SET @sql8 = @sql8 + N' AND st.uptYmd BETWEEN @i__sYmd1 AND @i__eYmd1 '

IF @i__noRealYN = 'Y'
	SET @sql8 = @sql8 + N' AND  ISNULL(noRealYN, ''N'') <> ''Y'''
IF @i__qtyZeroYN = 'Y'
	SET @sql8 = @sql8 + N' AND ISNULL(str.qtyWorkable, 0) <> 0 AND ISNULL(st.stockQty, 0) <> 0 '

--위에서 이걸로 변경. 2024.10.16 hsg
IF @i__logComCode NOT IN (SELECT comCode FROM dbo.UF_GetChildComcode(@ErpOperateComCode))   
BEGIN
	SET @sql8 = @sql8 + N' AND	nd.itemId IS NULL'
END
-------------------------------------------------------------------------------------------------------------------------------------------------------------ssy
-------------------------------------------------------------------------------------------------------------------------------------------------------------ssy
--sql9: 글로벌웍스코리아 재고
	...
	(
		...
		where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
		  AND (@i__consignCustCode = '''' or _s.consignCustCode = @i__consignCustCode 
		    or (_s.consignCustCode is null AND _s.comCode = @i__consignCustCode))
		  and _sr.stockQty > 0
		  AND _s.consignCustCode = ''ㄱ008''--글로벌웍스코리아
	'
	--if @i__logUserId = 'zzz'
	--	SET @sql = @sql + N'	and _s.storageCode <> ''zzz'' '
	SET @sql9 = @sql9 + N'	) locaMemo ,  '
END
ELSE
BEGIN
SET @sql9 = @sql9 + N''''' locaMemo ,'
...
,''ㄱ008'' AS otherSaleType

,ISNULL(osr.purRate , 0) AS saleRate
'
IF @i__itemBulk <> ''
BEGIN
    SET @sql9 = @sql9 + N' , ISNULL(bk.idx, NULL) AS bk_idx '
END
SET @sql9 = @sql9 + N' 

,ca3.qty3 AS qty3

,''ㄱ008'' AS stockRackCode
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty1 --글로벌웍스코리아 재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode AND _s.storageCode = _r.storageCode 
	where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
	AND (@i__consignCustCode = '''' or _s.consignCustCode = @i__consignCustCode 
	  or (_s.consignCustCode is null AND _s.comCode = @i__consignCustCode))
	AND _s.consignCustCode = ''ㄱ008''
    
) ca1

CROSS APPLY (
	select sum(_sr.stockQty) AS qty2 --글로벌웍스코리아 가용재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode AND _s.storageCode = _r.storageCode 
	where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
...
CROSS APPLY (
	select sum(_sr.stockQty) AS qty3 --글로벌웍스코리아,이지통상 제외 가용재고
	from dbo.e_stockRack _sr
	LEFT JOIN dbo.e_rack _r ON _sr.comCode = _r.comCode AND _sr.rackCode = _r.rackCode
	LEFT JOIN dbo.e_storage _s ON _s.comCode = _r.comCode AND _s.storageCode = _r.storageCode 
	where _sr.itemid = st.itemId and _sr.comCode = st.comCode 
	...
	and _s.validYN = ''Y'' AND _s.storType in (''신품'',''중고'',''리퍼'') 
	AND _s.workableYN = ''Y''
	AND ISNULL(_s.consignCustCode,'''') <> ''ㄱ008''
	AND ISNULL(_s.consignCustCode,'''') <> ''ㅇ496''
...
BEGIN
	IF @i__bulkSrchType = 'itemId' 
		SET @sql9 = @sql9 + '	INNER JOIN (SELECT srchKeyWord, MIN(idx) idx 
		                                    FROM #tbl_itemH 
											GROUP BY srchKeyWord) bk ON i.ItemId = bk.srchKeyword '
...
IF @i__storageCode <> '' 
BEGIN
	SET @sql9 = @sql9 + '	JOIN #stockItem3 st2 ON st2.comCode = st.comCode AND st2.itemId = st.itemId '
END
...
IF @i__logComCode NOT IN (SELECT comCode FROM dbo.UF_GetChildComcode(@ErpOperateComCode))  
BEGIN
	SET @sql9 = @sql9 + '	LEFT OUTER JOIN dbo.e_stockItemOuterNonDsp nd ON st.comCode = nd.comCode 
	                          AND st.itemId = nd.itemId '
END
...
IF @i__checkType = 'OUT'
BEGIN
	SET @sql9 = @sql9 + N' AND 1=1 '
END
ELSE IF @i__checkType = 'ALL' --외부재고다중조회
BEGIN
	SET @sql9 = @sql9 + N' AND @n__4carComCode = st.comCode  '
END
ELSE
BEGIN
	SET @sql9 = @sql9 + N' AND st.comCode = @i__logComCode '
END
...
IF @i__itemId <> ''
SET @sql9 = @sql9 + N' AND st.itemId= @i__itemId '

IF @i__itemNo <> ''
SET @sql9 = @sql9 + N' AND i.itemNo LIKE ''%'+@i__itemNo+'%'' '	

--IF @i__storName <> ''
--SET @sqlW = @sqlW + N'  AND sg.storageName LIKE '''+@i__storName+'%'' '	

IF @i__makerCode <> ''
SET @sql9 = @sql9 + N' AND i.makerCode= @i__makerCode '

IF @i__itemName <> ''
SET @sql9 = @sql9 + N' AND (i.itemName LIKE ''%'+@i__itemName+'%'' OR i.itemNameEn LIKE ''%'+@i__itemName+'%'' ) '

IF @i__classCode <> ''
SET @sql9 = @sql9 + N' AND i.classCode= @i__classCode '

IF @i__ymdIgnoreYN <> 'Y' AND @i__sYmd1 <> ''
	SET @sql9 = @sql9 + N' AND st.uptYmd BETWEEN @i__sYmd1 AND @i__eYmd1 '

IF @i__noRealYN = 'Y'
	SET @sql9 = @sql9 + N' AND  ISNULL(noRealYN, ''N'') <> ''Y'''
IF @i__qtyZeroYN = 'Y'
	SET @sql9 = @sql9 + N' AND ISNULL(str.qtyWorkable, 0) <> 0 AND ISNULL(st.stockQty, 0) <> 0 '

--위에서 이걸로 변경. 2024.10.16 hsg
IF @i__logComCode NOT IN (SELECT comCode FROM dbo.UF_GetChildComcode(@ErpOperateComCode))   
BEGIN
	SET @sql9 = @sql9 + N' AND	nd.itemId IS NULL'
END
-------------------------------------------------------------------------------------------------------------------------------------------------------------ssy
