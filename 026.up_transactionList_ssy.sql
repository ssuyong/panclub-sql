USE [panErp]
GO
/****** Object:  StoredProcedure [dbo].[up_transactionList_ssy]    Script Date: 2026-03-24 오전 10:51:38 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER  PROC [dbo].[up_transactionList_ssy]
/***************************************************************
설명 : 거래상세내역
...
--***************************************************************/
	...
	,@i__saleNo varchar(20) = '' --saleNo
	,@i__puComCode varchar(20) = '' --puComCode
...
INSERT INTO panErp.dbo._SPLOG( sp,params)
	VALUES('panErp.dbo.up_transactionList_ssy', 
	...
	@i__saleNo='''+ISNULL(@i__saleNo,'')+''',
    @i__puComCode='''+ISNULL(@i__puComCode,'')+''''	 
   )
---------------------------------
...
SET @i__saleNo = ISNULL(@i__saleNo, '')
SET @i__puComCode = ISNULL(@i__puComCode, '')
...
IF @i__workingType = 'WHLIST' --매입처 거래상세내역
	GOTO WHLIST_QRY  --리스팅
IF @i__workingType = 'WHITEMLIST' --매입처 거래상세 상세내역
	GOTO WHITEMLIST_QRY  --리스팅
...
join dbo.e_cust ec on ec.custCode = si.puComCode
  and ec.comCode = @i__logComCode
where si.saleNo = @i__saleNo
  and si.comCode = @i__puComCode
