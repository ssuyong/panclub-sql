...
declare @i__itemNo varchar(20) = '2218200959'
,@i__itemBulk varchar(MAX) = '2218200959'
...
ISNULL(ca2.qty2,0) AS workableQty,  
	(
		...
		and _sr.stockQty > 0
		AND _s.consignCustCode <> '¤·499' --¾Æ¿ìÅä¼­¿ï
		AND _s.consignCustCode <> '¤²022' --VAG ±ºÆ÷
		AND _s.consignCustCode <> '¤·479' --ÀÎÅÍÄ«½º ºÎ»ê
		AND _s.consignCustCode <> '¤·002' --¿¥ÄÉÀÌ ´ë±¸

		AND _s.consignCustCode <> '¤·496' --ÀÌÁöÅë»ó ³²¾çÁÖ
...
,dbo.UF_GREATEST(0 ,(ISNULL(str.qtyNewWorkable, 0) - ISNULL(temp.qtyNew,0)  - ISNULL(ca3.qty3,0) - ISNULL(ca4.qty4,0) - ISNULL(ca5.qty5,0)  - ISNULL(ca6.qty6,0) - ISNULL(ca7.qty7,0)
...
CROSS APPLY (
	...
	AND _s.consignCustCode <> '¤²022'
	AND _s.consignCustCode <> '¤·479'
...
LEFT OUTER JOIN dbo.e_stockItemOuterNonDsp nd ON st.comCode = nd.comCode AND st.itemId = nd.itemId --
WHERE 1 = 1
AND i.itemNo LIKE '%'+@i__itemNo+'%'
AND st.comCode = @i__logComCode
